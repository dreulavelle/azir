...
func TestFQDNReadsBothPlatforms(t *testing.T) {
	windows := "\ufeffServer:  dns.google\r\nAddress:  8.8.8.8\r\n\r\nName:    rainfirm.3cx.us\r\nAddress:  66.194.172.120\r\n"
	linux := "\ufeffServer:\t\t8.8.8.8\nAddress:\t8.8.8.8#53\n\nNon-authoritative answer:\nName:\tstonebridge.la.3cx.us\nAddress: 63.210.128.28\n"

	if got := readFQDN([]byte(windows)); got != "rainfirm.3cx.us" {
		t.Errorf("Windows lookup read as %q", got)
	}
	if got := readFQDN([]byte(linux)); got != "stonebridge.la.3cx.us" {

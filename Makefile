...
init: ## Generate any missing secrets in .env (never rewrites one that exists)
	...
	add AZIR_LOG_LEVEL "info" \
	  "# debug, info, warn or error. A development deployment usually wants\n# debug: it is the difference between a tool failing and being able to see\n# which subject it failed on."; \
